...
_SAR_TITLES = [
    ...
    "SAR Table",
    "Compound Series Overview",
    "Lead Optimisation: Analogue Series",
    "In Vitro SAR Data",
    "Medicinal Chemistry Summary",
]

_MMP_TITLES = [
    "Matched Molecular Pair Analysis",
    "Bioisostere Comparison",
    "SAR by MMP",
...
def make_data_card_page(
    ...
    label_box = draw_rotated_text(
        page, label, (label_cx, label_cy), id_font, 0.0, fill=fill
    )
...
def make_sar_page(
    ...
    t_font = load_font(font_paths, t_sz, prefer_bold=True)
    draw.text(
        (margin, margin), random.choice(_SAR_TITLES), font=t_font, fill=(20, 20, 20)
    )
    ...
    act_names = random.sample(
        list(_COLUMN_GENERATORS.keys()), min(n_act_cols, len(_COLUMN_GENERATORS))
    )
    ...
    r_labels = [
        "R\u2081" if i == 0 else "R\u2082" if i == 1 else f"R{i + 1}"
    ...
    for hx, hname in zip(col_xs, col_hdrs):
        draw.text(
            (hx + cell_pad, y_cursor + (hdr_h - hdr_sz) // 2),
            hname,
            font=hdr_font,
            fill=(30, 30, 30),
        )
...
def make_mmp_page(
    ...
    t_font = load_font(font_paths, t_sz, prefer_bold=True)
    draw.text(
        (margin, margin), random.choice(_MMP_TITLES), font=t_font, fill=(20, 20, 20)
    )
    y_cursor = margin + int(t_sz * 2.2)
    ...
    delta_names = random.sample(
        list(_COLUMN_GENERATORS.keys()), min(n_delta, len(_COLUMN_GENERATORS))
    )
    ...
    for _ in range(n_pairs):
        ...
        for idx, (smi, img) in enumerate([(smi_a, img_a), (smi_b, img_b)]):
            ...
            label_box = draw_rotated_text(
                page, label, (label_cx, label_cy), id_font, 0.0
            )
            panels.append(
                {
                    "struct_box": struct_box,
        ...
        draw.text(
            (arrow_x0 + (arrow_x1 - arrow_x0 - tw) // 2, arrow_y - th - int(8 * scale)),
            transform,
            font=ann_font,
            fill=(40, 40, 40),
        )
