...
from structflo.cser.generation.specialty import (
    make_data_card_page,
    make_mmp_page,
    make_sar_page,
)
...
def _generate_one_page(args: tuple) -> None:
    ...
    (i, split, img_dir, lbl_dir, gt_dir, fmt, dpi_choices, grayscale, page_seed) = args
    ...
    if roll < 0.13:  # 13 % slide layouts
        ...
        if roll < 0.21:  #  8 % hard negatives
            ...
            page, panels = make_data_card_page(
                _mp_smiles, cfg, _mp_fonts, _mp_distractors
            )
...
def generate_dataset(
    ...
    print(
        f"DPI choices: {dpi_choices}  |  Grayscale: {grayscale}  |  Workers: {effective_workers}"
    )
    ...
    if effective_workers <= 1:
        ...
        def run_split(
            count: int, img_dir: Path, lbl_dir: Path, gt_dir: Path, split: str
        ) -> None:
            for i in tqdm(range(count), desc=f"Generating {split}"):
                ...
                if roll < 0.13:
                    cfg = make_page_config_slide(min(dpi, 200))
                    page, panels = make_page(
                        smiles_pool, cfg, font_paths, distractor_pool
                    )
                else:
                    cfg = make_page_config(dpi)
                    if roll < 0.21:
                        page, panels = make_negative_page(
                            cfg, font_paths, distractor_pool
                        )
                    elif roll < 0.35:
                        page, panels = make_excel_page(
                            smiles_pool, cfg, font_paths, distractor_pool
                        )
                    elif roll < 0.47:
                        page, panels = make_grid_page(
                            smiles_pool, cfg, font_paths, distractor_pool
                        )
                    elif roll < 0.55:
                        page, panels = make_data_card_page(
                        ...
                        page, panels = make_sar_page(
                            smiles_pool, cfg, font_paths, distractor_pool
                        )
                    elif roll < 0.70:
                        page, panels = make_mmp_page(
                            smiles_pool, cfg, font_paths, distractor_pool
                        )
                    else:
                        page, panels = make_page(
                ...
                save_sample(
                    page, panels, img_path, lbl_path, gt_path, fmt, cfg, grayscale
                )
        ...
        run_split_parallel(
            num_train, train_img_dir, train_lbl_dir, train_gt_dir, "train", seed
        )
        run_split_parallel(
            num_val, val_img_dir, val_lbl_dir, val_gt_dir, "val", seed + num_train
        )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate synthetic dataset for structure detection"
    )
    parser.add_argument(
        "--smiles",
        type=Path,
    ...
    parser.add_argument(
        "--num-train", type=int, default=2000, help="Number of training pages"
    )
    parser.add_argument(
        "--num-val", type=int, default=200, help="Number of validation pages"
    )
    parser.add_argument("--seed", type=int, default=7, help="Random seed")
    parser.add_argument(
        "--format", choices=["jpg", "png"], default="jpg", help="Image format"
    )
    parser.add_argument(
        "--fonts-dir",
        type=Path,
